...
namespace Affdex
{
    ...
    public class CameraInput : MonoBehaviour, IDetectorInput
    {
        ...
        public Texture2D SavePhotoToDisk()
        {
            if (cameraTexture == null)
                return null;

            string directory = Application.dataPath + "/../Photos/"+applicationManager.status+"/";
            ...
            {
                File.WriteAllBytes(directory + "Snap_" + applicationManager.status + ".jpg", snap.EncodeToJPG());
                previewImage2.sprite = Sprite.Create(snap, new Rect(0, 0, snap.width, snap.height), new Vector2(0.5f, 0.5f), 125);
            }
            if (applicationManager.status == "PhotoBooth")
            {
                File.WriteAllBytes(directory + "Snap_" + applicationManager.status + "_" + DateTime.Now.ToFileTimeUtc() + ".jpg", snap.EncodeToJPG());
                previewImage.sprite = Sprite.Create(snap, new Rect(0, 0, snap.width, snap.height), new Vector2(0.5f, 0.5f), 125);
            }
